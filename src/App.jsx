...
const SHEET_URL = “https://opensheet.elk.sh/TU_SHEET_ID/productos”;
...
style={{ background: CARD_BG, borderRadius: 12, overflow: “hidden”, border: `1px solid ${inC ? GOLD : "#222"}`, cursor: “pointer”, transition: “border-color 0.15s”, display: “flex”, flexDirection: “column” }}>
{/* Image */}
...
<button
onClick={e => { e.stopPropagation(); if (stock > 0) addToCart(p._id, e); }}
...
{[
[t.price, `${parseFloat(selected.Precio || 0).toFixed(2)}€`],
[t.series, selected.Serie],
[“Categoría”, selected.Categoria],
[t.stock_label, selected.Stock],
...
<style>{`* { box-sizing: border-box; margin: 0; padding: 0; } input::placeholder { color: #444; } ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: #111; } ::-webkit-scrollbar-thumb { background: #333; border-radius: 2px; } .modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.8); z-index: 200; display: flex; align-items: flex-end; justify-content: center; } .modal-sheet { background: #141414; border-radius: 20px 20px 0 0; padding: 0 20px 44px; width: 100%; max-width: 560px; max-height: 93vh; overflow-y: auto; border-top: 1px solid #222; } .handle { width: 40px; height: 3px; background: #333; border-radius: 2px; margin: 12px auto 20px; } .bottom-bar { position: fixed; bottom: 0; left: 0; right: 0; background: #0a0a0a; border-top: 1px solid #1e1e1e; display: flex; padding-bottom: env(safe-area-inset-bottom, 16px); padding-top: 10px; z-index: 100; } .nav-item { flex: 1; display: flex; flex-direction: column; align-items: center; gap: 3px; cursor: pointer; padding: 4px; } .nav-label { font-size: 9px; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; } .row-item { background: ${CARD_BG}; padding: 13px 16px; border-bottom: 1px solid #1e1e1e; display: flex; align-items: center; gap: 12px; cursor: pointer; } .row-item:active { background: #1e1e1e; } .cart-bar { position: fixed; bottom: 80px; left: 0; right: 0; background: ${GOLD}; padding: 12px 20px; display: flex; justify-content: space-between; align-items: center; z-index: 90; }`}</style>
